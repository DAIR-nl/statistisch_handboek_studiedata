...
```
De data is verdeeld in 2 kolommen, het cijfer en het startjaar.

<<<<<<< HEAD
### Normaliteit
=======
### data beschrijven en normaliteit
selecteer beide groepen om het gemiddelde, de standaarddeviatie en het histogram uit te draaien.
```{r data selecteren}
Cijfers_2010 <- Cijfers_gem[Cijfers_gem$Startjaar==2010,]
Cijfers_2011 <- Cijfers_gem[Cijfers_gem$Startjaar==2011,]
```
>>>>>>> 2ca05aa4345e135a0ca860b523d0b229b9ef2fbb
<div class="col2">
```{r Assumptietest, collapse=TRUE}
mean(Cijfers_2010$Cijfers)
sd(Cijfers_2010$Cijfers)
hist(Cijfers_2010$Cijfers)
mean(Cijfers_2011$Cijfers)
sd(Cijfers_2011$Cijfers)
hist(Cijfers_2011$Cijfers)
```
</div>
...
lillie.test(Cijfers_2010$Cijfers)
lillie.test(Cijfers_2011$Cijfers)
```
</div>
...
### Levene's test
De `leveneTest()` heeft in R 2 argumenten. Het eerste argument is een vector met de data van beide groepen. Het tweede argument is een factor waarin de groepen worden gedefinieerd of een vector die logisch om te zetten is naar een factor. `Cijfers_gem$Startjaar` is een vector met twee waardes (2010 en 2011). Dit kan R makkelijk omzetten naar een factor bestaande uit de waardes 2010 en 2011.

...
hist = plt.hist(dfGemiddeld_cijfer_WNS, density = True, edgecolor = "black", bins = 9)
...
```{python qqplot}
qq = stats.probplot(dfGemiddeld_cijfer_WNS, dist="norm", plot=plt)
...
``` {python boxplot}
## Boxplot
box = plt.boxplot(dfGemiddeld_cijfer_WNS)
...
print(statsmodels.stats.api.lilliefors(dfGemiddeld_cijfer_WNS))
```
<!-- ## /OPENBLOK: Lilliefors-test.py -->
...
```{python ttest}
## T-test
print(stats.ttest_1samp(dfGemiddeld_cijfer_WNS, popmean = 6.8))
```
<!-- ## /OPENBLOK: T-test.py -->
```{python ttest als object, echo = FALSE}
stat, pval = stats.ttest_1samp(dfGemiddeld_cijfer_WNS, popmean = 6.8)
par = N - 1
```

...
## Boxplot
fig, ax = plt.subplots()
box = ax.boxplot([Uren_studeren_T0, Uren_studeren_T1], labels = ["T0", "T1"])
...
print(stats.shapiro(Uren_studeren_T0_n30))
```
<!-- ## /OPENBLOK: Shapiro-Wilk-test-1.py -->

<!-- ## OPENBLOK: Shapiro-Wilk-test-2.py -->
``` {python Shapiro-Wilk Test-2, warning=FALSE}
print(stats.shapiro(Uren_studeren_T1_n30))
```
<!-- ## /OPENBLOK: Shapiro-Wilk-test-2.py -->
...
df = N1 - 1
diffmean = np.mean(np.abs(np.array(Uren_studeren_T1) - np.array(Uren_studeren_T0)))
```
<!-- ## /CLOSEDBLOK: T-test.py -->
...
Om Cohen's D te kunnen berekenen, moet de onderstaande functie `cohen_d()` worden overgenomen, omdat deze niet een Python eigen functie is.[^11]
<!-- ## /TEKSTBLOK: Cohen-d.py  -->
...
Een *gepaarde t-toets* is uitgevoerd om te toetsen of het gemiddeld aantal uur studeren van de studenten is veranderd na deelname aan de cursus. Het verschil tussen het gemiddelde van T~0~ (*M~T0~* = `r Round_and_format(py$vMean_t0)`, *SD~T0~* = `r Round_and_format(py$vSD_t0)`) en het gemiddelde van T~1~ (*M~T1~* =`r Round_and_format(py$vMean_t1)`, *SD~T1~* = `r Round_and_format(py$vSD_t1)`) is significant, *t* ~`r py$df`~ = `r Round_and_format(py$stat)`, *p* < 0,01. Het valt op dat het gemiddelde van T~1~ hoger is dan van T~0~. Het effect is echter minimaal klein, *d* = `r Round_and_format(py$d)`. 

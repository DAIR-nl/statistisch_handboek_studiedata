...
## Voer de chi-kwadraat toets voor goodness of fit uit
chi2, pval = sps.chisquare(sp.array(Aantal_observaties), sp.array(Verwacht_aantal_observaties), axis = None)
...
* Significant hoger aantal observaties bij een negatief BSA , *z* = `r Round_and_format(py$PH_res[1])`
* Geen significant lager aantal observaties bij een positief BSA , *z* = `r Round_and_format(py$PH_res[2])`
* Geen significant verschillend aantal observaties bij een uitgesteld BSA , *z* = `r Round_and_format((py$PH_res[3])`
...
Uit de post-hoc toetsen blijkt dat het aantal studenten met een negatief BSA significant hoger is dan het percentage van de gehele universiteit (`r Round_and_format(py$Proptab[1, 1])`%, *z* = `r Round_and_format(py$PH_res[1])`). Het aantal studenten met een positief BSA (`r Round_and_format(py$Proptab[2, 1])`%, *z* = `r Round_and_format(py$PH_res[2])`) en een uitgesteld BSA (`r Round_and_format(py$Proptab[3, 1])`%, *z* = `r Round_and_format(py$PH_res[3])`) is niet significant verschillend is van het percentage van de gehele universiteit. De resultaten suggereren dat de opleiding Maritieme Technieken qua BSA dus iets afwijkt van de gehele universiteit waarbij het aantal negatieve BSA's hoger is in vergelijking tot de gehele universiteit.
<!-- ## /TEKSTBLOK: Chi-kwadraat5.py-->
...
<!-- ## CLOSEDNBLOK: Binomiaaltoets3_3.py -->
```{python, echo = FALSE}
...
## Bereken de aantallen observaties
Aantal_observaties = pd.crosstab(dfBSA_Maritieme_techniek_steekproef['BSA_advies'], columns = 'Frequentie')
Aantal_observaties = np.array(dfBSA_Maritieme_techniek_steekproef['BSA_advies'].value_counts())

## Zet het aantal observaties om in een numpy array. Gebruik de toevoeging .T om de array om te zetten in een rij
#Aantal_observaties = Aantal_observaties.T
#Aantal_observaties = Aantal_observaties['Frequentie']
## Sla de verwachte proporties op in een vector
Verwachte_proporties = np.array([0.2,0.7, 0.1])
...
t1 = met.Multinom(Verwacht_aantal_observaties, Aantal_observaties)
t1_p1 = t1.onesided_exact_test() 
t1_p2 = t1.twosided_exact_test()
print(t1_p1)
print(t1_p2)

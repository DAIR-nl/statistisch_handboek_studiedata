...
<!-- ## /CLOSEDBLOK: Data-beschrijven-2.R -->
<!-- ## TEKSTBLOK: Data-beschrijven.R-->
* Mediaan inkomens op T~1~: `r format(vMed_T1, scientific = FALSE)` 
* Mediaan inkomens op T~2~: `r format(vMed_T2, scientific = FALSE)` 
...
### Boxplot
De box geeft de middelste 50% van de tentamencijfers weer. De zwarte lijn binnen de box is de mediaan. De staarten bevatten de eerste 25% en de laatste 25%. Cirkels visualiseren mogelijke uitbijters, echter uitbijters zijn niet direct van invloed op de uitkomst van de *Wilcoxon signed rang toets*.[^8] Hoe meer de boxen overlappen, hoe waarschijnlijker dat er geen significant verschil is tussen de groepen.  
...
```{r wilcox test}
wilcox.test(Alumni_inkomens_T2, Alumni_inkomens_T1, 
...
```{r effectgrootte}
# Sla p-waarde op
pwaarde <- wilcox.test(Alumni_inkomens_T2, Alumni_inkomens_T1, 
...
r <- qnorm(pwaarde) / sqrt(nrow(Alumni_inkomens))
...
De *Wilcoxon signed rank toets* is uitgevoerd om de vraag te beantwoorden of de inkomens van de alumni van de Academie Mens & Maatschappij in vier jaar tijd is gestegen. De mediaan van de inkomens van 1 jaar na afstuderen is `r format(vMed_T1, scientific = FALSE)` per maand. De mediaan van de inkomens van 5 jaar na afstuderen is `r format(vMed_T2, scientific = FALSE)` per maand. Uit de resultaten kan afgelezen worden dat er een statistisch significant verschil is tussen de inkomens van de alumni van de Academie Mens & Maatschappij bij T~0~ en T~1~, *V* = `r vVstatistic`, *p* < 0,0001, *r* = `r Round_and_format(r)`.
<!-- ## /TEKSTBLOK: Rapportage.R -->

...
<!-- ## /OPENBLOK: Data-aanmaken.R -->
# Toepassing
Gebruik de *Mann-Whitney U toets* om te toetsen of de verdelingen twee onafhankelijke groepen significant van elkaar verschillen.[^1] De *Mann-Whitney U toets* kan een alternatief zijn voor de [ongepaarde t-toets](03-Ongepaarde-t-toets-R.html) als de verdelingen niet normaal verdeeld zijn. Alleen als de verdelingen van beide groepen dezelfde vorm hebben, kan de *Mann-Whitney U toets* gebruikt worden om het verschil tussen de medianen van twee groepen te toetsen.[^9] Gebruik *Mood's mediaan toets* om medianen te toetsen bij twee verdelingen die niet dezelfde vorm hebben.
...
Het meetniveau van de afhankelijke variabele is ordinaal of continu.[^9] In deze toetspagina staat een casus met continue data centraal; een casus met ordinale data is te vinden in een andere toetspagina.

De *Mann-Whitney U toets* stelt minder eisen aan de data en hoeft onder andere niet te voldoen aan de assumptie van normaliteit.  Bij de *Mann-Whitney U toets* hebben uitbijters minder invloed op het eindresultaat dan bij de [ongepaarde t-toets](03-Ongepaarde-t-toets-R.html). Daarentegen, als de data wel normaal verdeeld zijn heeft de *Mann-Whitney U toets* minder onderscheidend vermogen dan de [ongepaarde t-toets](03-Ongepaarde-t-toets-R.html).[^2] Vandaar dat ondanks de voordelen er toch minder vaak voor de *Mann-Whitney U toets* gekozen wordt.

## Verdeling steekproeven

De *Mann-Whitney U toets* schrijft geen assumpties voor over de verdeling van de verschilscores (verschillen tussen beide meetmomenten voor alle observatie-eenheden).[^5] In principe toetst de *Mann-Whitney U toets* een hypothese over het verschil tussen de verdelingen van twee ongepaarde groepen. Als de verdelingen van beide ongepaarde groepen echter dezelfde vorm hebben, toetst de *Mann-Whitney U toets* ook een hypothese over het verschil tussen de medianen. Immers, het enige verschil tussen de verdeling is in dat geval een verschuiving van de verdeling, dus een verandering van de mediaan. In dat geval heeft de *Mann-Whitney U toets* een hoger onderscheidend vermogen[^4] dan *Mood's mediaan toets* om medianen te toetsen.[^10]

Als de verdelingen van de groepen niet dezelfde vorm hebben, doet de *Mann-Whitney U toets* een uitspraak over het verschil tussen verdelingen. Een verschil tussen verdelingen kan meerdere oorzaken hebben. De top van de verdelingen kan verschillend zijn, maar ook de spreiding van de verdeling kan verschillen.[^9] De *Mann-Whitney U toets* maakt een rangschikking van alle observaties van beide groepen samengevoegd en telt vervolgens apart de rangschikkingen op voor de observaties in beide groepen. Het verschil tussen de som van de rangschikkingen in beide groepen bepaalt de significantie van de toets.[^10] Met behulp van de groepsgroottes kan ook de gemiddelde rangschikking van beide groepen berekend worden. Vanuit dat perspectief kan de *Mann-Whitney U toets* gezien worden als een toets die de gemiddelde rangschikking van twee groepen vergelijkt. Benoem daarom de gemiddelde rangschikking van beide groepen in de rapportage en visualiseer de verdeling van beide groepen om duidelijk te maken op welke manier de verdelingen van elkaar verschillen.
...
Bereken ten slotte de gemiddelde rangschikking van beide groepen.
...
* De gemiddelde rangschikking is `r Round_and_format(Rangschikkingen_Nederlandse_vooropleiding)` (*n*=`r vN_NL`) voor studenten met een Nederlandse vooropleiding en `r Round_and_format(Rangschikkingen_buitenlandse_vooropleiding)` (*n*=`r vN_Inter`) voor studenten met een buitenlandse vooropleiding  
...
[^11]: Miller, I. & Miller, C. (2012). *John E. Freund's Mathematical Statistics with Applications*. Pearson: eighth edition.

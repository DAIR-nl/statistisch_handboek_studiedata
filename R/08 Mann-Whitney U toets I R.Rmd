...
<!-- ## /OPENBLOK: Data-aanmaken.R -->
# Toepassing
Gebruik de *Mann-Whitney U toets* om te toetsen of de gemiddelde rangschikkingen van de verdelingen van twee ongepaarde groepen van elkaar verschillen.[^1] De *Mann-Whitney U toets* is een alternatief voor de [ongepaarde t-toets](03-Ongepaarde-t-toets-R.html) als de verdelingen niet normaal verdeeld zijn. Alleen als de verdelingen van beide groepen dezelfde vorm hebben, kan de *Mann-Whitney U toets* ook gebruikt worden om het verschil tussen de medianen van twee groepen te toetsen.[^9] Gebruik *Mood's mediaan toets* om medianen te toetsen bij twee ongepaarde groepen waarvan de verdelingen niet dezelfde vorm hebben.
...
*H~0~*: Er is geen verschil in de gemiddelde rangschikking van het behaalde aantal studiepunten in het tweede jaar tussen studenten met een buitenlandse vooropleiding en een Nederlandse vooropleiding.

*H~A~*: Er is een verschil in de gemiddelde rangschikking van het behaalde aantal studiepunten in het tweede jaar tussen studenten met een buitenlandse vooropleiding en een Nederlandse vooropleiding. Eén van beide verdelingen bevat hogere waarden betreffende het behaalde aantal studiepunten.
</div>

# Assumpties

Het meetniveau van de afhankelijke variabele is ordinaal[^12] of continu.[^9] In deze toetspagina staat een casus met continue data centraal; een casus met ordinale data is te vinden in een andere toetspagina.
...
De *Mann-Whitney U toets* schrijft geen assumpties voor over de verdeling van de twee ongepaarde groepen.[^9] In principe toetst de *Mann-Whitney U toets* een hypothese over het verschil tussen de gemiddelde rangschikking van de verdelingen van twee ongepaarde groepen. De *Mann-Whitney U toets* maakt een rangschikking van alle observaties van beide groepen samengevoegd en telt vervolgens apart de rangschikkingen op voor de observaties in beide groepen. Met behulp van de groepsgroottes kan ook de gemiddelde rangschikking van beide groepen berekend worden. Het verschil tussen de gemiddelde rangschikkingen in beide groepen bepaalt de significantie van de toets.[^10]  Vanuit dat perspectief kan de *Mann-Whitney U toets* gezien worden als een toets die de gemiddelde rangschikking van twee groepen vergelijkt.

Als de verdelingen van de groepen niet dezelfde vorm hebben, doet de *Mann-Whitney U toets* een uitspraak over het verschil tussen verdelingen. Een verschil tussen verdelingen kan meerdere oorzaken hebben. De top of toppen van de verdelingen kunnen verschillend zijn, maar ook de spreiding van de verdeling kan verschillen.[^9] In alle gevallen is er echter een verschil tussen de gemiddelde rangschikking van de verdelingen. In andere woorden, de ene verdeling bevat hogere waarden dan de andere verdeling.  Benoem daarom de gemiddelde rangschikking van beide groepen in de rapportage en visualiseer de verdeling van beide groepen om duidelijk te maken op welke manier de verdelingen van elkaar verschillen.
...
Maak een histogram om de verdeling van het aantal studiepunten in het tweede jaar voor studenten met een Nederlandse en buitenlandse vooropleiding visueel weer te geven.[^13]
...
Bereken ten slotte de gemiddelde rangschikking van beide groepen. Beoordeel op basis van de gemiddelde rangschikkingen welke groep hogere waardes bevat.
<!-- ## OPENBLOK: Wilcoxon-signed-rank-toets3.R -->
```{r sommen, collapse = TRUE}
Rangschikkingen_Nederlandse_vooropleiding <- mean(rank(Studiepunten_studiejaar2$Studiepunten)[Vooropleiding_Nederlands])

Rangschikkingen_buitenlandse_vooropleiding <- mean(rank(Studiepunten_studiejaar2$Studiepunten)[!Vooropleiding_Nederlands])

# Print gemiddelde rangschikkingen
Rangschikkingen_Nederlandse_vooropleiding
Rangschikkingen_buitenlandse_vooropleiding
```
<!-- ## /OPENBLOK: Wilcoxon-signed-rank-toets3.R -->
...
* De gemiddelde rangschikking is `r Round_and_format(Rangschikkingen_Nederlandse_vooropleiding)` (*n*=`r vN_NL`) voor studenten met een Nederlandse vooropleiding en `r Round_and_format(Rangschikkingen_buitenlandse_vooropleiding)` (*n*=`r vN_Inter`) voor studenten met een buitenlandse vooropleiding. De verdeling van studenten met een Nederlandse vooropleiding bevat dus hogere waarden dan de verdeling van studenten met een buitenlandse vooropleiding.

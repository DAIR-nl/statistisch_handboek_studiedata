...
Een effectmaat geeft informatie over de grootte van het significante resultaat
dat in een studie is gevonden. De *Mann-Whitney U toets* heeft als effectmaat
*r*.[^4] Een indicatie om *r* te interpreteren is: rond 0,1 is het een klein effect, 
rond 0,3 is het een gemiddeld effect en rond 0,5 is het een groot effect.[^5]

# Uitvoering 
<!-- ## TEKSTBLOK: Dataset-inladen.R-->
Er is een dataset `Studiepunten_jaar2` ingeladen met het aantal punten dat studenten in het tweede jaar halen.
<!-- ## /TEKSTBLOK: Dataset-inladen.R-->
...
head(Studiepunten_jaar2)
## Eerste 6 observaties
tail(Studiepunten_jaar2)
```
<!-- ## /OPENBLOK: Data-bekijken.R -->
...
```{r data selecteren}
Nederlands <- Studiepunten_jaar2$Vooropleiding == "Nederland"
```
<!-- ## /OPENBLOK: Data-selecteren.R -->
...
length(Studiepunten_jaar2$Studiepunten[Nederlands])
quantile(Studiepunten_jaar2$Studiepunten[Nederlands])
```
<!-- ## /OPENBLOK: Data-beschrijven-1.R -->
  </div>
  <div class="col">
...
x <- group_by(Studiepunten_jaar2, Vooropleiding) %>%
  summarise(
    N = n(),
...
Gebruik `wilcox.test()` om een *Mann-Whitney U toets* te doen. Het eerste argument bevat het aantal studiepunten van studenten met een Nederlandse vooropleiding `Studiepunten_jaar2$Studiepunten[Nederlands]`; het tweede argument het aantal studiepunten van studenten met een buitenlandse vooropleiding `Studiepunten_jaar2$Studiepunten[!Nederlands]`.
<!-- ## /TEKSTBLOK: Mann-Whitney-U-toets-1.R -->

<!-- ## OPENBLOK: Mann-Whitney-U-toets-2.R -->
```{r Mann Whitney U toets}
wilcox.test(Studiepunten_jaar2$Studiepunten[Nederlands], 
            Studiepunten_jaar2$Studiepunten[!Nederlands],
            paired = FALSE,
...
wcx <- wilcox.test(Studiepunten_jaar2$Studiepunten[Nederlands], 
            Studiepunten_jaar2$Studiepunten[!Nederlands],
            paired = FALSE,
            alternative = "two.sided",
            conf.int = TRUE)
...
Bereken de effectgrootte *r* vervolgens op basis van de *p*-waarde van de *Mann-Whitney U toets*.
```{r effectgrootte}
# Sla p-waarde op
pwaarde <- wilcox.test(Studiepunten_jaar2$Studiepunten[Nederlands], 
            Studiepunten_jaar2$Studiepunten[!Nederlands],
            paired = FALSE,
            alternative = "two.sided",
            conf.int = TRUE)$p.value

# Bereken effectgrootte tweezijdige test
r <- qnorm(pwaarde/2) / sqrt(nrow(Studiepunten_jaar2))
# Bereken effectgrootte eenzijdige test
#r <- qnorm(pwaarde) / sqrt(nrow(Studiepunten_jaar2))

# Print effectgrootte
paste("Effectgrootte is",Round_and_format(r))

```
<!-- ## /OPENBLOK: Mann-Whitney-U-toets-4.R -->

<!-- ## TEKSTBLOK: Mann-Whitney-U-toets-4.R -->
* *W* = `r vW_W`, *p* = < 0,0001, *r* = `r Round_and_format(r)`
* *p*-waarde < 0,05, dus de H~0~ wordt niet verworpen.[^6]
* Effectgrootte is `r Round_and_format(r)`, dus een klein tot gemiddeld effect

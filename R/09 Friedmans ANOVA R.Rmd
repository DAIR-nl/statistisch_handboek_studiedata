...
vN_SC <- Round_and_format(x[4,2])
vN_CC <- Round_and_format(x[3,2])

vQ1_HG <- Round_and_format(x[1,3])
vQ1_BG <- Round_and_format(x[2,3])
vQ1_SC <- Round_and_format(x[4,3])
vQ1_CC <- Round_and_format(x[3,3])

vMed_HG <- Round_and_format(x[1,4])
vMed_BG <- Round_and_format(x[2,4])
vMed_SC <- Round_and_format(x[4,4])
vMed_CC <- Round_and_format(x[3,4])

vQ3_HG <- Round_and_format(x[1,5])
vQ3_BG <- Round_and_format(x[2,5])
vQ3_SC <- Round_and_format(x[4,5])
vQ3_CC <- Round_and_format(x[3,5])
```
<!-- ## /CLOSEDBLOK: Data-beschrijven.R -->
...
* Er is een significant verschil tussen de beoordelingen van de eetgelegenheden in de vier gebouwen; het effect van de verschillende gebouwen op de beoordelingen van studenten is gemiddeld

<!-- ## /TEKSTBLOK: ANOVA-toets1.R -->

## Post-hoc toets
<!-- ## TEKSTBLOK: posthoc1.R -->
Voer post-hoc toetsen uit om te onderzoeken tussen welke gebouwen er verschillen zijn in de beoordelingen van studenten. De vier gebouwen zijn het hoofdgebouw, bestuursgebouw, sportcentrum en cultuurcentrum Rembrandt. Gebruik de *Wilcoxon signed-rank toets* als post-hoc toets met bijbehorende functie `pairwise.wilcox.test()`. De ingevoerde argumenten van de functie zijn de afhankelijke variabele `Beoordelingen_eetgelegenheden$Beoordeling`, de onafhankelijke variabele `Beoordelingen_eetgelegenheden$Eetgelegenheid` die de groepen aangeeft, de gebruikte methode om te corrigeren voor meerdere toetsen `p.adjust.method = "bonferroni"`, het argument `paired = TRUE` om aan te geven dat er een gepaarde steekproef is en het argument `exact = FALSE` om aan te geven dat de p-waarden niet op een exacte manier berekend worden. Het laatste argument wordt gebruikt omdat de exacte methode in bepaalde gevallen niet mogelijk is.    
<!-- ## /TEKSTBLOK: posthoc1.R -->

<!-- ## OPENBLOK: post-hoc2.R -->
``` {r pairwise wilcox test}
pairwise.wilcox.test(Beoordelingen_eetgelegenheden$Beoordeling, Beoordelingen_eetgelegenheden$Eetgelegenheid,
                     p.adjust.method = "bonferroni", paired = TRUE, exact = FALSE)
```
<!-- ## /OPENBLOK: post-hoc2.R -->


<!-- ## CLOSEDBLOK: post-hoc3.R -->
``` {r pairwise wilcox test 2}
posthoc <- pairwise.wilcox.test(Beoordelingen_eetgelegenheden$Beoordeling, Beoordelingen_eetgelegenheden$Eetgelegenheid,
...
* De beoordelingen van het hoofdgebouw verschillen niet significant van het bestuursgebouw (*p* = `r Round_and_format(posthoc[2,1])`), niet significant van het sportcentrum (*p* = `r Round_and_format(posthoc[3,3])`) en wel significant van het cultuurcentrum (*p* = `r Round_and_format(posthoc[2,2])`)
* De beoordelingen van het bestuursgebouw verschillen niet significant van het hoofdgebouw (*p* = `r Round_and_format(posthoc[2,1])`), niet significant van het sportcentrum (*p* = `r Round_and_format(posthoc[3,1])`) en niet significant van het cultuurcentrum (*p* = `r Round_and_format(posthoc[1,1])`)
* De beoordelingen van het sportcentrum verschillen niet significant van het hoofdgebouw (*p* = `r Round_and_format(posthoc[3,3])`), niet significant van het bestuursgebouw (*p* = `r Round_and_format(posthoc[3,1])`) en wel significant van het cultuurcentrum (*p* = `r Round_and_format(posthoc[3,2])`)
* De beoordelingen van het cultuurcentrum verschillen wel significant van het hoofdgebouw (*p* = `r Round_and_format(posthoc[2,2])`), niet significant van het bestuursgebouw (*p* = `r Round_and_format(posthoc[1,1])`) en wel significant van het sportcentrum (*p* = `r Round_and_format(posthoc[3,2])`)
...
Een *repeated measures ANOVA* is uitgevoerd om te onderzoeken of er verschillen zijn tussen het aantal studieuren dat eerstejaars studenten van de bachelor Technische Natuurkunde besteden aan de vakken in de eerste onderwijsperiode. De vakken zijn Lineaire Algebra, Relativiteitstheorie en Kosmologie; beschrijvende statistieken zijn te vinden in Tabel 1. De data voldoen niet aan de assumptie van sphericiteit ($\chi^2$ = 3, p < 0,0001), daarom is de Greenhouse-Geisser correctie gebruikt. Het aantal studieuren voor de drie vakken in de eerste onderwijsperiode verschilt significant van elkaar, *F* = 3, p < 0,0001, *η^2^* = 3. De sterkte van het effect van het vak op het aantal studieuren is groot. Post-hoc toetsen - waarbij een Bonferroni correctie is toegepast om te corrigeren voor meerdere toetsen -  tonen aan dat elk van de drie de vakken significant verschilt van de andere twee vakken.

| Eetgelegenheid | Mediaan     | Q1        |  Q3        | N        |
| -------------  | ----------  | --------- | ---------- | -------- |
| Hoofdgebouw    | `r vMed_HG` | `r vQ1_HG`| `r vQ3_HG` | `r vN_HG`|
| Bestuursgebouw | `r vMed_BG` | `r vQ1_BG`| `r vQ3_BG` | `r vN_BG`|
| Sportcentrum   | `r vMed_SC` | `r vQ1_SC`| `r vQ3_SC` | `r vN_SC`|
| Cultuurcentrum | `r vMed_CC` | `r vQ1_CC`| `r vQ3_CC` | `r vN_CC`|
...
[^7]: *Kendall's W* wordt berekend door de test statistiek van *Friedman's ANOVA* (dit is de &chi;^2) te delen door het aantal observatie-eenheden *N* en het aantal groepen *k* minus één, i.e. $W = \frac{\chi^2}{N(k-1)}$.

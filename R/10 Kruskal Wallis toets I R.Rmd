...
# Toepassing
Gebruik de *Kruskal Wallis toets* om te toetsen of de verdelingen van twee of meer groepen van elkaar verschillen.[^1] De *Kruskal Wallis toets* kan een alternatief zijn voor de [one-way ANOVA](05-One-way-ANOVA-R.html).[^2] De *Kruskal Wallis toets* stelt minder eisen aan de data; die hoeft onder andere niet te voldoen aan de assumptie normaliteit. Bij de *Kruskal Wallis toets* hebben uitbijters minder invloed op het eindresultaat dan bij de [one-way ANOVA](05-One-way-ANOVA-R.html). Daarentegen, als de data wel normaal verdeeld zijn, heeft de *Kruskal Wallis toets* minder onderscheidend vermogen dan de [one-way ANOVA](05-One-way-ANOVA-R.html).[^3]<sup>, </sup>[^4] Vandaar dat ondanks de voordelen er toch minder vaak voor de *Kruskal Wallis toets* test gekozen wordt. 
...
Gebruik `kruskal.test()`. Gebruik als eerste argument de afhankelijke variabele `EC_Jaar1` en de variabele die de groep definiëert: `Vooropleiding`. Het tweede argument is het dataframe. 
<!-- ## /TEKSTBLOK: Kruskal-Wallis-test-1.R -->
...
Gebruik de [Mann-Whitney U toets](08-Mann-Whitney-U-toets-R.html) als post-hoc toets om te bepalen wélke groepen significant verschillen. De [Mann-Whitney U toets](08-Mann-Whitney-U-toets-R.html) wordt ook wel de *Wilcoxon rank-sum toets* genoemd.[^15] Gebruik de functie `pairwise.wilcox.test()` met als eerste argument de afhankelijke variabele `Resultaten_Arbeidsrecht$EC_Jaar1` en als tweede argument de definitie van de groepen `Resultaten_Arbeidsrecht$Vooropleiding`. Pas de *Bonferroni correctie* toe met `p.adjust.method = "bonferroni"`. Naast de p-waarde toont de [Mann-Whitney U toets](08-Mann-Whitney-U-toets-R.html) het verschil in mediaan en bijbehorend betrouwbaarheidsinterval als resultaat en *r* als effectmaat. Voor meer informatie, zie de toetspagina van de [Mann-Whitney U toets](08-Mann-Whitney-U-toets-R.html).   
<!-- ## /TEKSTBLOK: Mann-Whitney-U-test.R -->
...
Gemiddelde_rangschikking <- function(Vooropleiding_1, Vooropleiding_2){
  ...
  Gemiddelde_rangschikking_Vooropleiding_1 <- mean(Rangschikkingen[Groepsindicator == 1])
  Gemiddelde_rangschikking_Vooropleiding_2 <- mean(Rangschikkingen[Groepsindicator == 2])
  # Retourneer beide gemiddelde rangschikkingen
  return(list(Groep_1 = Gemiddelde_rangschikking_Vooropleiding_1, Groep_2 = Gemiddelde_rangschikking_Vooropleiding_2))
}
...
Gem_FR_NR <- Gemiddelde_rangschikking(Studiepunten_Fiscaal_Recht, Studiepunten_Notarieel_Recht)

Gem_FR_PM <- Gemiddelde_rangschikking(Studiepunten_Fiscaal_Recht, Studiepunten_Premaster)

Gem_FR_RG <- Gemiddelde_rangschikking(Studiepunten_Fiscaal_Recht, Studiepunten_Rechtsgeleerdheid)

Gem_NR_PM <- Gemiddelde_rangschikking(Studiepunten_Notarieel_Recht, Studiepunten_Premaster)

Gem_NR_RG <- Gemiddelde_rangschikking(Studiepunten_Notarieel_Recht, Studiepunten_Rechtsgeleerdheid)

Gem_PM_RG <- Gemiddelde_rangschikking(Studiepunten_Premaster, Studiepunten_Rechtsgeleerdheid)
...
| -------------  | ----------  | ---------- | -------- |
| FR vs. NR    | `r Round_and_format(posthoc[1,1])` |  `r Round_and_format(Gem_FR_NR[[1]])` | `r Round_and_format(Gem_FR_NR[[2]])` |
| FR vs. PM    | `r Round_and_format(posthoc[2,1])` |  `r Round_and_format(Gem_FR_PM[[1]])` | `r Round_and_format(Gem_FR_PM[[2]])` |
| FR vs. RG    | `r Round_and_format(posthoc[3,1])` |  `r Round_and_format(Gem_FR_RG[[1]])` | `r Round_and_format(Gem_FR_RG[[2]])` |
| NR vs. PM    | `r Round_and_format(posthoc[2,2])` |  `r Round_and_format(Gem_NR_PM[[1]])` | `r Round_and_format(Gem_NR_PM[[2]])` |
| NR vs. RG    | `r Round_and_format(posthoc[3,2])` |  `r Round_and_format(Gem_NR_RG[[1]])` | `r Round_and_format(Gem_NR_RG[[2]])` |
| PM vs. RG    | `r Round_and_format(posthoc[3,3])` |  `r Round_and_format(Gem_PM_RG[[1]])` | `r Round_and_format(Gem_PM_RG[[2]])` |
...
[^15]: Van Geloven, N. (13 maart 2018).  *Mann-Whitney U toets*. [Wiki Statistiek Academisch Medisch Centrum](https://wikistatistiek.amc.nl/index.php/Mann-Whitney_U_toets).

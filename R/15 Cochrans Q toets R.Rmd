...
Voer post-hoc toetsen uit om te onderzoeken tussen welke onderwijsperiodes er verschillen zijn in het percentage eerstejaars studenten met herkansingen. Gebruik de *McNemar toets* of exacte *McNemar toets* als post-hoc toets. De exacte versie van de *McNemar toets* is nuttig om te gebruiken wanneer het aantal observaties in de niet-diagonale elementen van de confusiematrix kleiner dan 11 is. Omdat er bij post-hoc toetsen vaak meerdere vergelijkingen gemaakt worden, is het tijdrovend om voor elke idnividuele vergelijking de confusiematrix te inspecteren. Voer daarom de *McNemar toets* en de exacte *McNemar toets* beide uit en vergelijk de significantie op beide toetsen. Wanneer een vergelijking op de ene toets significant is en op de andere toets niet significant, bepaal dan aan de hand van de confusiematrix welk toetsresultaat gebruikt moet worden.

Voer de post-hoc toetsen uit met de functie `pairwiseMcnemar()` met als eerste argument `Herkansingen ~ Onderwijsperiode | Studentnummer` waarin `Herkansingen` de afhankelijke variabele is, `Onderwijsperiode` de variabele is die de verschillende groepen aangeeft en `Studentnummer` de variabele is die de observatie-eenheden aangeeft. Het tweede argument is de dataset `data = Herkansingen_kunstmatige_intelligentie` en het vierde argument `method = "BH"` geeft de correctie voor meerdere toetsen weer (Benjamini-Hochberg (BH) in dit geval). Het derde argument `test = "mcnemar"` geeft aan dat de *McNemar toets* gebruikt moet worden; gebruik het argument `test = "exact"` om een exacte *McNemar toets* uit te voeren. Het significantieniveau is 0,05 voor beide toetsen.[^10]
<!-- ## /TEKSTBLOK: posthoc1.R -->

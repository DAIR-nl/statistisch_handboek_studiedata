...
Met *multilevel multinomiale logistische regressie* wordt getoetst of er een afhankelijkheid is tussen de keuze voor een specialisatie en de verschillende onderwijsperiodes. Voer een post-hoc toets uit om daarna te bepalen welke onderwijsperiodes van elkaar verschillen wat betreft de keuze voor specialisaties. Gebruik hiervoor de [McNemar toets](12-McNemar-toets-R.html) of de exacte [McNemar toets](12-McNemar-toets-R.html) als post-hoc toets.[^6]
...
Fit het gereduceerde model met de functie `brm()` met als argument `formula = Specialisatie ~ (1 | Studentnummer)` met als afhankelijke variabele `specialisatie` en `(1 | Studentnummer)` om een student specifiek intercept in te stellen, `data = Data_Specialisatie` voor de gebruikte dataset, `family = "categorical"` om aan te geven dat de afhankelijke variabele nominaal is, een seed `seed = 12345` zodat de uitkomsten bij opnieuw runnen van de code hetzelfde zijn en `save_all_pars = TRUE` zodat de loglikelihood berekend kan worden.

Fit het uitgebreide model met de functie `brm()` met als argument `formula = Specialisatie ~ Onderwijsperiode + (1 | Studentnummer)` met als afhankelijke variabele `Specialisatie`, als onafhankelijke variabele `Onderwijsperiode` en `(1 | Studentnummer)` om een student specifiek intercept in te stellen, `data = Data_Specialisatie` voor de gebruikte dataset, `family = "categorical"` om aan te geven dat de afhankelijke variabele nominaal is, een seed `seed = 12345` zodat de uitkomsten bij opnieuw runnen van de code hetzelfde zijn en `save_all_pars = TRUE` zodat de loglikelihood berekend kan worden.
...
# Fit gereduceerde model: student specifiek intercept
fit1 <- brm(formula = Specialisatie ~ (1 | Studentnummer), 
           data = Data_Specialisatie, family = "categorical",
           seed = 12345, save_all_pars = TRUE)

# Fit uitgebeide model: student specifiek intercept plus onderwijsperiodes
fit2 <- brm(formula = Specialisatie ~ Onderwijsperiode + (1 | Studentnummer), 
           data = Data_Specialisatie, family = "categorical",
           seed = 12345, save_all_pars = TRUE)
```
```{r, cache=TRUE}
...
LogLikelihood_Functie <- function(fit){
  # Sla matrix met loglikelihood samples op
  LL1 <- log_lik(fit)
  # Neem exponent om likelihood in plaats van loglikelihood te krijgen
  LL2 <- exp(LL1)
  # Bereken gemiddelde likelihood per kolom (per student) met colMeans
  LL3 <- colMeans(LL2)
  # Neem logaritme om terug te gaan naar log-likelihood
  LL4 <- log(LL3)
  # Neem de som zodat je loglikelihood voor alle studenten krijgt
  LL5 <- sum(LL4)
  # Retourneer de loglikelihood
  return(LL5)
}

# Bereken loglikelihood voor beide modellen
Loglikelihood_Model_1 <- LogLikelihood_Functie(fit1)
Loglikelihood_Model_2 <- LogLikelihood_Functie(fit2)

# Bereken toetsstatistiek van likelihood ratio toets
Toetsstatistiek <- -2 * (Loglikelihood_Model_1 - Loglikelihood_Model_2)

# Stel aantal predictors in dat is verwijderd bij het gereduceerde model
Vrijheidsgraden <- 8

# Bereken p-waarde
p_waarde <- 1 - pchisq(Toetsstatistiek, df = Vrijheidsgraden)

# Print toetsstatistiek en p-waarde
Toetsstatistiek
p_waarde
```
<!-- ## /OPENBLOK: MMLR2.R -->
...
Voer post-hoc toetsen uit om te onderzoeken tussen welke onderwijsperiodes er verschillen zijn in de verdeling van de keuzes voor de specialisaties van de eerstejaars studenten Psychologie. Gebruik de [McNemar toets](12-McNemar-toets-R.html) of exacte [McNemar toets](12-McNemar-toets-R.html) als post-hoc toets. De exacte versie van de [McNemar toets](12-McNemar-toets-R.html) is nuttig om te gebruiken wanneer het aantal observaties in de niet-diagonale elementen van de confusiematrix kleiner dan 11 is. Omdat er bij post-hoc toetsen vaak meerdere vergelijkingen gemaakt worden, is het tijdrovend om voor elke idnividuele vergelijking de confusiematrix te inspecteren. Voer daarom de [McNemar toets](12-McNemar-toets-R.html) en de exacte [McNemar toets](12-McNemar-toets-R.html) beide uit en vergelijk de significantie op beide toetsen. Wanneer een vergelijking op de ene toets significant is en op de andere toets niet significant, bepaal dan aan de hand van de confusiematrix welk toetsresultaat gebruikt moet worden.

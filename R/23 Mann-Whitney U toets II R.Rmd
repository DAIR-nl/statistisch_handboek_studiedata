...
<!-- ## /OPENBLOK: Data-aanmaken.R -->
# Toepassing
Gebruik de *Mann-Whitney U toets* om te toetsen of twee ongepaarde groepen van elkaar verschillen op een ordinale variabele. Een alternatief voor een vergelijking van twee groepen op een ordinale variabele is de *chi-kwadraat toets voor onafhankelijkheid* of de *Fisher-Freeman-Halton exact toets* (bij een laag aantal observaties). Bij deze toetsen wordt echter geen rekening gehouden met de ordening van de categorieën van de ordinale variabele: de variabele wordt behandeld als een nominale variabele.
...
Voer de *Mann-Whitney U toets* uit om de vraag te beantwoorden of er verschillen zijn tussen de beoordelingen van de gesprekstechnieken van mannen en vrouwen. Zet eerst de categorische variabele `Beoordeling` om in een numerieke variabele door de categorieën onvoldoende, voldoende en goed om te zetten in 1, 2 en 3 respectievelijke. Gebruik daarna de functie `wilcox.test()` met als eerste argument `Beoordeling ~ Geslacht` waarin `Beoordeling` de afhankelijke variabele is en `Geslacht` de onafhankelijke variabele. Voer daarna het argument `paired = FALSE` in omdat de steekproeven ongepaard zijn en het argument `alternative = "two.sided"` vanwege de tweezijdige alternatieve hypothese

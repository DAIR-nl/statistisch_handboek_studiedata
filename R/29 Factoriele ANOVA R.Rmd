...
*H~A~*: Het gemiddelde cijfer is niet gelijk voor mannen en vrouwen, i.e. er is een hoofdeffect van Geslacht op Gemiddeld cijfer.
...
Voer een *simple effects analyse* uit om het interactie-effect te interpreteren. Vergelijk eerst de verschillen tussen de drie vooropleiding voor elke categorie van de variabele Geslacht (mannen en vrouwen dus). Vergelijk daarna de verschillen tussen mannen en vrouwen van elke categorie van de variabele Vooropleiding (hbo-p, vwo of overig).

### Vooropleiding

<!-- ## TEKSTBLOK: SimEff1.R -->
Maak eerst een aparte dataset voor mannen en vrouwen aan en voer vervolgens de Games-Howell post hoc toets uit met behulp van de functie `games_howell_test` van het package `rstatix`. Het eerste argument is de dataset (bijvoorbeeld `Mannen_Gemiddelde_cijfers_psychologie`) en het tweede argument is de formule `Gemiddeld_cijfer ~ Vooropleiding` met de afhankelijke variabele `Gemiddeld_cijfer` en de onafhankelijke variabele `Vooropleiding`.
<!-- ## /TEKSTBLOK: SimEff1.R -->
...
games_howell_test(Mannen_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Vooropleiding)
games_howell_test(Vrouwen_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Vooropleiding)
```
<!-- ## /OPENBLOK: SimEff2.R -->
...
# Voer voor beide datasets de Games-Howell post-hoc toets uit
GH_man <- games_howell_test(Mannen_Gemiddelde_cijfers_psychologie, 
                            Gemiddeld_cijfer ~ Vooropleiding)
GH_vrouw <- games_howell_test(Vrouwen_Gemiddelde_cijfers_psychologie, 
                              Gemiddeld_cijfer ~ Vooropleiding)
...
* Hbo-p versus vwo: het verschil in gemiddelde (vwo - hbo-p) is `r Round_and_format(GH_man$estimate[1])`, dit is een significant verschil (*p* < 0,0001).
* Hbo-p versus overig: het verschil in gemiddelde (overig - hbo-p) is `r Round_and_format(GH_man$estimate[2])`, dit is een significant verschil (*p* < 0,01).
* Vwo versus overig: het verschil in gemiddelde (overig - vwo) is `r Round_and_format(GH_man$estimate[3])`, dit is geen significant verschil (*p* = 1,00).

Voor vrouwen zijn de volgende vergelijkingen getoetst[^18]:

* Hbo-p versus vwo: het verschil in gemiddelde (vwo - hbo-p) is `r Round_and_format(GH_vrouw$estimate[1])`, dit is geen significant verschil (*p* = `r Round_and_format(GH_vrouw$p.adj[1])`).
* Hbo-p versus overig: het verschil in gemiddelde (overig - hbo-p) is `r Round_and_format(GH_vrouw$estimate[2])`, dit is geen significant verschil (*p* = `r Round_and_format(GH_vrouw$p.adj[2])`).
...
games_howell_test(Hbo_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Geslacht)
games_howell_test(Vwo_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Geslacht)
games_howell_test(Overig_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Geslacht)
```
<!-- ## /OPENBLOK: SimEff6.R -->
...
<!-- ## TEKSTBLOK: SimEff8.R -->
* Hbo-p: het verschil in gemiddelde (vrouw - man) is `r Round_and_format(GH_hbo$estimate[1])`, dit is een significant verschil (*p* < 0,0001).[^18]
* Vwo: het verschil in gemiddelde (vrouw - man) is `r Round_and_format(GH_vwo$estimate[1])`, dit is een significant verschil (*p* = `r Round_and_format(GH_vwo$p.adj[1])`).[^18]
* Overig: het verschil in gemiddelde (vrouw - man) is `r Round_and_format(GH_overig$estimate[1])`, dit is een significant verschil (*p* = `r Round_and_format(GH_overig$p.adj[1])`).[^18]
...
games_howell_test(Hoofdeffecten_Gemiddelde_cijfers_psychologie, 
                  Gemiddeld_cijfer ~ Vooropleiding)
...
GH_Vooropleiding <- games_howell_test(Hoofdeffecten_Gemiddelde_cijfers_psychologie, 
                                      Gemiddeld_cijfer ~ Vooropleiding)

GH_Geslacht <- games_howell_test(Hoofdeffecten_Gemiddelde_cijfers_psychologie, 
                                 Gemiddeld_cijfer ~ Geslacht)
...
* Hbo-p versus vwo: het verschil in gemiddelde (vwo - hbo-p) is `r Round_and_format(GH_Vooropleiding$estimate[1])`, dit is een significant verschil (*p* < 0,0001).
* Hbo-p versus overig: het verschil in gemiddelde (overig - hbo-p) is `r Round_and_format(GH_Vooropleiding$estimate[2])`, dit is een significant verschil (*p* < 0,001).

...
Toets deze assumptie met *Mauchly's test*. Als de data niet aan de assumptie voldoen, geeft de gewone *repeated measures ANOVA* een verkeerd resultaat. Er zijn echter correcties die gebruikt kunnen worden wanneer de data niet aan sphericiteit voldoen. Een voorbeeld van mogelijke output van *Mauchly's test* in R voor de huidige toetspagina is hieronder weergegeven.
...
Houd de volgende richtlijnen aan bij het interpreteren van de *factoriële repeated measures ANOVA* wanneer niet aan sphericiteit is voldaan:
...
*Figuur 3.  Gemiddelde van de gemiddelde cijfers per groep op basis van Vaksoort en Periode voor de dataset Resultaten_Leisure_Management.*
...
* Voor de onafhankelijke variabele Periode is er voldaan aan de assumptie van sphericiteit, *W* = `r Mauchly_W_Periode`, *p* = `r Round_and_format(p_Periode)`[^18]
* Voor de interactieterm van Periode en Vaksoort is er voldaan aan de assumptie van sphericiteit, *W* = `r Mauchly_W_Int`, *p* = `r Round_and_format(p_Int)`
...
* Vwo: het verschil in gemiddelde (vrouw - man) is `r Round_and_format(GH_vwo$diff[1])`, dit is een significant verschil (*p* = `r Round_and_format(GH_vwo$p[1])`).[^18]
* Overig: het verschil in gemiddelde (vrouw - man) is `r Round_and_format(GH_overig$diff[1])`, dit is een significant verschil (*p* = `r Round_and_format(GH_overig$p[1])`).[^18]

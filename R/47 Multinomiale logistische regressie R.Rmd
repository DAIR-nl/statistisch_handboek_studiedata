...
Bij *multinomiale logistische regressie* is de effectmaat gebaseerd op een vergelijking van de likelihood van het voorgestelde model ten opzichte van de likelihood van het interceptmodel. Een veelgebruikte versie hiervan is Nagelkerke's $R^2$ ($R_N^2$) welke waardes aanneemt tussen 0 en 1. Let op dat Nagelkerke's $R^2$ niet geïnterpreteerd kan worden als verklaarde variantie (zoals $R^2$ bij *multipele lineaire regressie*). Een juiste interpretatie is de verklaarde variatie of model fit.[^2]
...
Om een valide resultaat te vinden met *multinomiale logistische regressie*, dient er aan een aantal assumpties voldaan te worden.[^1]<sup>,</sup>[^2] De assumpties worden allen toegelicht en de opties bij het niet voldoen aan de assumptie worden weergegeven.
...
Bij *multinomiale logistische regressie* is de regressievergelijking zo opgesteld dat er een lineaire relatie is tussen elke predictor en de logit van de afhankelijke variabele. Een lineaire relatie houdt in dat de toename of afname van de (logit van de) afhankelijke variabele als gevolg van de toename van de predictor constant is. In tegenstelling tot *multipele lineaire regressie* kan bij *multinomiale logistische regressie* geen grafiek gemaakt worden om een lineaire relatie te onderzoeken. In plaats daarvan is er een statistische toets om te onderzoeken of er een lineaire relatie is tussen de predictor en de logit van de afhankelijke variabele. Bij deze *Box-Tidwell test* wordt er voor elke continue predictor een extra term toegevoegd die bestaat uit het product van de predictor en de logaritme van de predictor. Voor de predictor leeftijd zou dit dus betekenen dat er een extra variabele wordt (Leeftijd * ln[Leeftijd]). Voeg deze interactietermen voor alle continue predictors toe en toets of de regressiecoëfficiënten van deze interactietermen significant zijn. Als dit zo is, dan is er voor die predictor niet voldaan aan de assumptie van lineariteit. Een oplossing hiervoor is het transformeren[^5] van de variabele.[^6]
...
Fysiotherapie_Uitstroom_geprepareerd <- dfidx(Fysiotherapie_Uitstroom, 
                                              shape = "wide", 
                                              choice = "Uitstroom")

Regressiemodel <- mlogit(Uitstroom ~  1 |  1 + Studiepunten + Geslacht_dummy + Gemiddeld_cijfer,
                         Fysiotherapie_Uitstroom_geprepareerd)
...
boxplot(Fysiotherapie_Uitstroom[,c("Studiepunten", 
                                  "Gemiddeld_cijfer")],
        names = c("Studiepunten",
                  "Gemiddeld cijfer"))
...
Outlier_standaardiseren(Fysiotherapie_Uitstroom$Studiepunten)
Outlier_standaardiseren(Fysiotherapie_Uitstroom$Gemiddeld_cijfer)
...
<!-- ## OPENBLOK: Outlier4.R -->
```{r}
table(Fysiotherapie_Uitstroom$Uitstroom)
table(Fysiotherapie_Uitstroom$Geslacht)
```
<!-- ## /OPENBLOK: Outlier4.R -->
...
Fysiotherapie_Uitval_Niet_nominaal <- Fysiotherapie_Uitstroom[Fysiotherapie_Uitstroom$Uitstroom != "Nominaal",]
Fysiotherapie_Uitval_Nominaal <- Fysiotherapie_Uitstroom[Fysiotherapie_Uitstroom$Uitstroom != "Niet nominaal",]
Fysiotherapie_Nominaal_Niet_nominaal <- Fysiotherapie_Uitstroom[Fysiotherapie_Uitstroom$Uitstroom != "Uitval",]

## Los voor logistische regressiemodellen
Regressiemodel_Uitval_Niet_nominaal <- glm(Uitstroom ~ Studiepunten + Geslacht + Gemiddeld_cijfer,
                                           data = Fysiotherapie_Uitval_Niet_nominaal,
                                           family = "binomial")
Regressiemodel_Uitval_Nominaal <- glm(Uitstroom ~ Studiepunten + Geslacht + Gemiddeld_cijfer,
                                      data = Fysiotherapie_Uitval_Nominaal,
                                      family = "binomial")
Regressiemodel_Nominaal_Niet_nominaal <- glm(Uitstroom ~ Studiepunten + Geslacht + Gemiddeld_cijfer,
                                             data = Fysiotherapie_Nominaal_Niet_nominaal,
                                             family = "binomial")


# Sla de gestandaardiseerde residuën op
Residu_gestandaardiseerd_Uitval_Niet_nominaal <- rstandard(Regressiemodel_Uitval_Niet_nominaal)
Residu_gestandaardiseerd_Uitval_Nominaal <- rstandard(Regressiemodel_Uitval_Nominaal)
Residu_gestandaardiseerd_Nominaal_Niet_nominaal <- rstandard(Regressiemodel_Nominaal_Niet_nominaal)

# Plot de gestandaardiseerde residuën
plot(Residu_gestandaardiseerd_Uitval_Niet_nominaal, 
     xlab = "Volgorde", ylab = "Gestandaardiseerde residuën")
plot(Residu_gestandaardiseerd_Uitval_Nominaal, 
     xlab = "Volgorde", ylab = "Gestandaardiseerde residuën")
plot(Residu_gestandaardiseerd_Nominaal_Niet_nominaal, 
     xlab = "Volgorde", ylab = "Gestandaardiseerde residuën")

# Tel het aantal gestandaardiseerde residuën met een absolute waarde groter dan 3
sum(abs(Residu_gestandaardiseerd_Uitval_Niet_nominaal > 3))
sum(abs(Residu_gestandaardiseerd_Uitval_Nominaal > 3))
sum(abs(Residu_gestandaardiseerd_Nominaal_Niet_nominaal > 3))
...
Fysiotherapie_Uitstroom$Geslacht_dummy <- as.numeric(Fysiotherapie_Uitstroom$Geslacht == "Vrouw")

# Maak een subset van de dataset met alle predictors
Subset <- Fysiotherapie_Uitstroom[,c("Geslacht_dummy",
                                    "Gemiddeld_cijfer",
                                    "Studiepunten"
                                    )]
...
Cooks_afstand_Uitval_Niet_nominaal <- cooks.distance(Regressiemodel_Uitval_Niet_nominaal)
Cooks_afstand_Uitval_Nominaal <- cooks.distance(Regressiemodel_Uitval_Nominaal)
Cooks_afstand_Nominaal_Niet_nominaal <- cooks.distance(Regressiemodel_Nominaal_Niet_nominaal)

# Plot Cook's afstanden
plot(Cooks_afstand_Uitval_Niet_nominaal, 
     xlab = "Volgorde", ylab = "Cook's afstanden")
plot(Cooks_afstand_Uitval_Nominaal, 
     xlab = "Volgorde", ylab = "Cook's afstanden")
plot(Cooks_afstand_Nominaal_Niet_nominaal, 
     xlab = "Volgorde", ylab = "Cook's afstanden")

# Tel het aantal Cook's afstanden groter dan de criteriumwaarde van 1
sum(abs(Cooks_afstand_Uitval_Niet_nominaal > 1))
sum(abs(Cooks_afstand_Uitval_Nominaal > 1))
sum(abs(Cooks_afstand_Nominaal_Niet_nominaal > 1))
...
Multicollineariteit <- lm(log(Studiepunten) ~ Studiepunten + Geslacht + Gemiddeld_cijfer,
                           Fysiotherapie_Uitstroom)
...
Bereken de dispersieparameter om te bepalen of er sprake is van overdispersie. Voer hiervoor de *multinomiale logistische regressie* uit met behulp van de functie `glm()` met als derde argument `family = "quasibinomial"` die ervoor zorgt dat de resultaten gecorrigeerd worden voor de dispersie. Het eerste argument de regressievergelijking `Uitval ~ Geslacht + Eindexamencijfer +  Vooropleiding + Leeftijd` met links van de tilde de afhankelijke variabele `Uitval` en rechts de vier onafhankelijke variabelen `Geslacht`, `Eindexamencijfer`, `Vooropleiding` en `Leeftijd`. Het tweede argument is de dataset `Uitval_Elektrotechniek`.
<!-- ## /TEKSTBLOK: Dispersie1.R -->
...
De *likelihood ratio toets* wordt bij *multinomiale logistische regressie* gebruikt om de significantie van het gehele regressiemodel te toetsen ten opzichte van het interceptmodel. Deze *likelihood ratio toets* kan ook gebruikt worden om te toetsen of twee geneste regressiemodellen onderling verschillen qua model fit. Hoewel deze vergelijking in de casus niet voorbijkomt, wordt deze toch geïllustreerd. Vergelijk hiervoor een model met de predictoren Eindexamencijfer en Vooropleiding met een model met de predictors Geslacht, Eindexamencijfer, Vooropleiding en Leeftijd.

<!-- ## TEKSTBLOK: ModellenVergelijken1.R -->
Voer de *likelihood ratio toets* uit met de functie `lrtest()` van het package `lmtest` met als argumenten de modelobjecten van de resultaten van de twee regressiemodellen die worden vergeleken (`Model_1` en `Model_2`). Voer eerst de *multinomiale logistische regressie* uit voor beide modellen en vergelijk ze daarna. Bereken ten slotte de Nagelkerke $R^2$ met de functie `NagelkerkeR2(Model_1)$R2` met als argument het modelobject waarvan de verklaarde variatie berekend moet worden.
<!-- ## /TEKSTBLOK: ModellenVergelijken1.R -->
...
# Bereken de verklaarde variantie (%) van model 1
library(fmsb)
100*NagelkerkeR2(Model_1)$R2

# Bereken de verklaarde variantie (%) van model 2
...
Een *multinomiale logistische regressie* is uitgevoerd om te onderzoeken of de vooropleiding, de leeftijd, het geslacht en het gemiddeld eindexamencijfer van studenten die starten met de bachelor Elektrotechniek gerelateerd is aan het wel of niet uitvallen in het eerste jaar van deze opleiding. Uit het toetsen van de assumpties van *multinomiale logistische regressie* bleek dat er aan alle assumpties is voldaan. De resultaten van de *multinomiale logistische regressie* zijn te vinden in Tabel 1. 

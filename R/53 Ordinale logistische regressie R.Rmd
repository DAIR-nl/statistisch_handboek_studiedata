...
<!-- ## OPENBLOK: Outlier8.R -->
```{r}
# Zet Vooropleiding om in een numerieke variabele met een 1 voor een vrouw en 0 voor een man
...
# Indien nodig, verkrijg de significantie van de regressiecoëfficiënten na correctie voor de dispersie
summary(vvv3, dispersion = Dispersie_parameter)
...
Er is sprake van een onderdispersie, aangezien de dispersieparameter (`r Round_and_format(Dispersieparameter)`) kleiner dan 1 is. Het verschil met 1 is echter niet zeer klein en bij onderdispersie wordt er in het algemeen geen correctie uitgevoerd. Voor de *ordinale logistische regressie* in deze casus wordt daarom geen correctie uitgevoerd vanwege onderdispersie.
<!-- ## /TEKSTBLOK: Dispersie3.R -->
...
# Laad het package voor de likelihood ratio toets
library(lmtest)

# Toets de significantie van het verschil tussen beide modellen
lrtest(Proportional_odds, Geen_proportional_odds)
...
# Toets de significantie van het verschil tussen beide modellen
PO_toets <- lrtest(Proportional_odds, Geen_proportional_odds)@Body
PO_toets$`Pr(>Chisq)`
...
Uit de *likelihood ratio toets* blijkt dat er geen significant verschil is tussen beide modellen, *&chi;^2^* = `r Round_and_format(PO_toets$Chisq[2])`, *df* = `-PO_toets$Df[2]`, *p* = `r Round_and_format(PO_toets$`Pr(>Chisq)`,3)`. Er is dus voldaan aan de assumptie van *proportional odds*.
...
# Toets de significantie van het verschil tussen beide modellen
lrtest(Proportional_odds, Geen_proportional_odds_vooropleiding)
lrtest(Proportional_odds, Geen_proportional_odds_leeftijd)
lrtest(Proportional_odds, Geen_proportional_odds_gemiddeld_cijfer)
...
LR_toets <- lrtest(Regressiemodel)@Body
Deviance_interceptmodel <- -2*LR_toets$LogLik[2]
Deviance_regressiemodel <- -2*LR_toets$LogLik[1]
Aantal_observaties <- nrow(Beoordelingen_eindproject)

Nagelkerke_R2 <- (1 - exp( (Deviance_regressiemodel - Deviance_interceptmodel) / Aantal_observaties) ) / (1 - exp( -Deviance_interceptmodel / Aantal_observaties) )
...
<!-- ## TEKSTBLOK: Ftoets1.R -->
Bepaal als eerste stap de significantie van het regressiemodel met behulp van de *likelihood ratio toets*. De *likelihood ratio toets* voor het regressiemodel laat een significant verschil zien tussen het voorgestelde model en een model met alleen een intercept, *&chi;^2^* = `r Round_and_format(LR_toets$Chisq[2])`, *df* = `r LR_test$Df[2]`, *p* < 0,0001, $R^2_N$ = `r Round_and_format(100 * Nagelkerke_R2)`.[^8] De nulhypothese dat geen enkele predictor van de predictors Vooropleiding, Leeftijd en  Gemiddeld_cijfer gerelateerd is aan de afhankelijke variabele Beoordeling kan verworpen worden. De Nagelkerke $R^2$ is `r Round_and_format(100*R2)`. Omdat de *likelihood ratio toets* voor het gehele model significant is, kunnen de coëfficiënten geinterpreteerd worden.
<!-- ## TEKSTBLOK: Ftoets1.R -->
...
LR_toets_model_1 <- lrtest(Model_1)@Body
Deviance_interceptmodel <- -2*LR_toets_model_1$LogLik[2]
Deviance_model_1 <- -2*LR_toets_model_1$LogLik[1]
Aantal_observaties <- nrow(Beoordelingen_eindproject)

Nagelkerke_R2_model_1 <- (1 - exp( (Deviance_model_1 - Deviance_interceptmodel) / Aantal_observaties) ) / (1 - exp( -Deviance_interceptmodel / Aantal_observaties) )

Nagelkerke_R2_model_1

# Bereken de verklaarde variatie (%) van model 2
LR_toets_model_2 <- lrtest(Model_2)@Body
Deviance_interceptmodel <- -2*LR_toets_model_2$LogLik[2]
Deviance_model_2 <- -2*LR_toets_model_2$LogLik[1]
Aantal_observaties <- nrow(Beoordelingen_eindproject)

Nagelkerke_R2_model_2 <- (1 - exp( (Deviance_model_2 - Deviance_interceptmodel) / Aantal_observaties) ) / (1 - exp( -Deviance_interceptmodel / Aantal_observaties) )
...
| ------------------------- | ---------| ---------| ---------| ---------| ---------| ---------| ---------| 
| Intercept; hbo master vs. werk         | `r Round_and_format(Reg[1,1])` |  `r Round_and_format(Reg[1,2])` |  `r Round_and_format(Reg[1,3])` |  < 0,0001*  |  `r Round_and_format(CI[1,1])` - `r Round_and_format(CI[1,2])`  | - | - |
| Intercept; wo master vs. werk         | `r Round_and_format(Reg[2,1])` |  `r Round_and_format(Reg[2,2])` |  `r Round_and_format(Reg[2,3])` |  < 0,0001*  |  `r Round_and_format(CI[2,1])` - `r Round_and_format(CI[2,2])`  | - | - |
| Nominaal (Nominaal); hbo master vs. werk | `r Round_and_format(Reg[3,1])` |  `r Round_and_format(Reg[3,2])` |  `r Round_and_format(Reg[3,3])` |   0,007* |  `r Round_and_format(CI[3,1])` - `r Round_and_format(CI[3,2])` | `r Round_and_format(Coefficienten_std[3])`| `r Round_and_format(exp(Reg[3,1]))` |
| Nominaal (Nominaal); wo master vs. werk | `r Round_and_format(Reg[4,1])` |  `r Round_and_format(Reg[4,2])` |  `r Round_and_format(Reg[4,3])` |  `r Round_and_format(Reg[4,4])` |  `r Round_and_format(CI[4,1])` - `r Round_and_format(CI[4,2])` | `r Round_and_format(Coefficienten_std[4])`| `r Round_and_format(exp(Reg[4,1]))` |
| Gemiddeld_cijfer; hbo master vs. werk       | `r Round_and_format(Reg[5,1])` |  `r Round_and_format(Reg[5,2])` |  `r Round_and_format(Reg[5,3])` |  < 0,0001* |  `r Round_and_format(CI[5,1])` - `r Round_and_format(CI[5,2])` | `r Round_and_format(Coefficienten_std[5])`| `r Round_and_format(exp(Reg[5,1]))` |
| Gemiddeld_cijfer; wo master vs. werk       | `r Round_and_format(Reg[6,1])` |  `r Round_and_format(Reg[6,2])` |  `r Round_and_format(Reg[6,3])` |  < 0,0001* |  `r Round_and_format(CI[6,1])` - `r Round_and_format(CI[6,2])` | `r Round_and_format(Coefficienten_std[6])`| `r Round_and_format(exp(Reg[6,1]))` |
...
[^15]: De Nagelkerke $R^2$ is te berekenen met de formule $R^2_N = \frac{1 + exp([Dev_{model} - Dev_{null}]/N)}{1 + exp([- Dev_{null}]/N)}$ waarbij $Dev_{model}$ de deviance van het getoetste model is, $Dev_{null}$ de deviance van een interceptmodel (verschillend per vergelijking) is en $N$ het aantal deelnemers is. Voor meer informatie zie Tabachnick, B.G. & Fidell, L.S. (2013). *Using multivariate statistics*. Sixth Edition, Pearson. Pagina 443-446. 
